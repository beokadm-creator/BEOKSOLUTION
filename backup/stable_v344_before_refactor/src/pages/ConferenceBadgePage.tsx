...
const ConferenceBadgePage: React.FC = () => {
  ...
  useEffect(() => {
      ...
      setMsg("데이터 조회 중...");
      const q = query(collection(db, 'registrations'), where('userId', '==', auth.user.id), where('slug', '==', slug));
      
      const unsub = onSnapshot(q, (snap) => {
          if (snap.empty) { setUiData(null); setMsg("등록 정보 없음"); return; }
          
          const raw = snap.docs[0].data();
          // EXTREME SANITIZATION
          setUiData({
              status: String(raw.attendanceStatus || 'OUTSIDE'),
              name: String(raw.userName || '이름 없음'),
              aff: String(raw.affiliation || raw.userAffiliation || '소속 없음'),
              id: String(snap.docs[0].id || 'ERR'),
              issued: !!raw.badgeIssued
          });
          setMsg(""); // Clear msg
      });
      return () => unsub();
