...
export const useRegistration = (conferenceId: string, user: ConferenceUser | null) => {
    ...
    const autoSave = async (step: number, formData: any, regId?: string | null) => {
        if (!user || !conferenceId) return null;
        ...
        try {
            ...
            if (regId) {
                ...
                // Strategy: Use a deterministic ID for draft "DRAFT_{userId}"?
                // Or just create a new one if not passed.
                // For simplicity, we should rely on resumeRegistration to find it, 
                // but if we are saving for the first time, we create one.
                // Let's use a "DRAFT" ID pattern or just create a new ID and return it.
                // BUT, if we create a new ID, we must return it so the component can store it.
                
                // Better: Use `resumeRegistration` logic to find existing draft first?
                // No, that's too many reads.
                // The Component should maintain `currentRegId`.
                
                // Fallback: If no regId, create one.
        ...
        try {
            ...
            const newRegId = await runTransaction(db, async (transaction) => {
                ...
                const newRegistration: Registration = {
                    ...
                    userId: user.id,
                    conferenceId,
                    paymentStatus: 'PAID', // Assuming instant success for now
                    paymentMethod,
                    amount,
                    refundAmount: 0,
                    receiptNumber,
