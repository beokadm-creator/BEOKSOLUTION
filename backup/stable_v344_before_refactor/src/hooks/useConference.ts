...
export const useConference = () => {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
        if (isMounted) {
            console.error('[useConference] Timeout: Fetching data took too long.');
            setData(prev => ({ 
                ...prev, 
                loading: false, 
    ...
    const fetchConferenceData = async () => {
      try {
        ...
        console.log(`[useConference] Fetching: Society=${societyId || '(Any)'}, Slug=${slug}`);
        ...
        if (!found) {
            // Silently fail here, let the UI handle error state based on null info
            console.warn(`Conference not found for slug: ${slug}`);
            if (isMounted) {
        ...
        console.error('[useConference] Error:', err);
        if (isMounted) {
