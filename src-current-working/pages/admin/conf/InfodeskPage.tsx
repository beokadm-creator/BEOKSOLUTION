...
const InfodeskPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        try {
            ...
            const newSettings = { ...parsed, infodesk: { design, option: issueOption } };
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center relative mt-16">
                ...
                <div className={`p-10 rounded-3xl w-full max-w-3xl shadow-2xl backdrop-blur-sm border transition-all duration-500 ${design.bgImage ? 'bg-black/40 border-white/20 text-white' : 'bg-green-50 border-green-200 text-green-900'
                    }`}>
                    <h2 className="text-5xl font-black mb-6">등록 확인 및 명찰 발급</h2>
                    <p className="opacity-80 mb-8 text-2xl font-medium">
                        등록 교환권(QR)을 스캐너에 인식시켜주세요.<br />(Please scan your Registration Voucher)
                    </p>
                ...
                {(scannerState.status === 'SUCCESS' || scannerState.status === 'ERROR') && (
                    <div className={`absolute inset-0 z-[60000] flex flex-col items-center justify-center animate-in fade-in zoom-in duration-200 ${scannerState.status === 'SUCCESS' ? 'bg-green-600' : 'bg-red-600'
                        ...
                        {scannerState.status === 'SUCCESS' ? <CheckCircle className="w-40 h-40 mb-8 drop-shadow-lg" /> : <AlertCircle className="w-40 h-40 mb-8 drop-shadow-lg" />}
