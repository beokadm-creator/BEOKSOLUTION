...
const SocietyLandingPage: React.FC = () => {
    ...
    const [, setAuthRefresh] = useState(0); // Force re-render when auth changes

    // [FIX-2026-01-21] Force component re-render when auth state changes
    useEffect(() => {
            ...
            <nav className="fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-100 h-20">
                <div className="max-w-7xl mx-auto px-6 h-full flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-6">
                        {(() => {
                            const isLoading = authHook.auth.loading;
                            const hasUser = !!authHook.auth.user;

                            if (isLoading) {
                                return <div className="w-32 h-10 animate-pulse bg-slate-200 rounded-xl" />;
                            }
                            ...
                            if (hasUser) {
                                return (
                                    <div className="flex items-center gap-3">
                                        ...
                                        <button
                                            onClick={() => authHook.logout()}
                                            className="bg-red-50 text-red-600 px-4 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 hover:bg-red-100 transition border border-red-200"
                                            title="로그아웃"
                                        >
                                            <LogOut size={16} />
                ...
                {upcomingConfs.length > 0 && (
                    <section>
                        <div className="flex items-center gap-4 mb-12">
                            ...
                            <div>
                                <h3 className="text-3xl font-black text-slate-900 tracking-tight">{UI_TEXT.upcomingPlans.title.ko}</h3>
                                <p className="text-slate-400 font-black text-[10px] uppercase tracking-[0.2em] mt-1">{UI_TEXT.upcomingPlans.subtitle.ko}</p>
                            </div>
                        </div>
                ...
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-12 text-center md:text-left">
                    <div className="space-y-4">
                        ...
                        <p className="max-w-xs text-[10px] font-bold text-slate-300 leading-relaxed uppercase tracking-widest">
                            본 학술 플랫폼은 이레지(eRegi)의 솔루션을 통해 <br /> 운영되고 있습니다. {FOOTER_INFO.companyKr}.
                        </p>
                    </div>
