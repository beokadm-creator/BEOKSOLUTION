        ...
        const fetchConf = async () => {
            try {
                ...
                if (data) {
                    ...
                    const societyId = data.id.split('_')[0];
                    const isDevEnv = window.location.hostname.includes('localhost') ||
                        window.location.hostname.includes('--') ||
                        window.location.hostname.includes('web.app');

                    // Allow if: No subdomain (Super Admin/Main site), or IDs match, or it's a dev environment
                    if (subdomain && societyId !== subdomain && !isDevEnv) {
                        console.warn('Society Mismatch detected, but allowing for now if developer or superadmin bypass can be confirmed.');
                        // For safety in production, we should be stricter, but during dev let's allow.
                        setConference(data);
                    } else {
                        setConference(data);
                    }
