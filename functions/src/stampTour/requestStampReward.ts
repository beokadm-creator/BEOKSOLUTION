...
import { CompletionRule, getRequiredCount, resolveSelectableRewards, selectReward, StampTourConfig } from "./shared";
        ...
        return db.runTransaction(async (tx) => {
            ...
            if (config.drawMode === "ADMIN") {
                throw new functions.https.HttpsError(
                    "failed-precondition",
                    "관리자 추첨 모드입니다. 운영 화면에서 추첨을 진행해 주세요."
                );
            }
            ...
            if (config.rewardFulfillmentMode === "LOTTERY") {
                throw new functions.https.HttpsError(
                    "failed-precondition",
                    "예약 추첨형 미션입니다. 지정된 추첨 시간 이후 관리자 추첨 결과를 확인해 주세요."
                );
            }
            ...
            const progress = progressSnap.exists ? progressSnap.data() as {
                rewardStatus?: string;
            ...
            if (selectableRewards.length === 0) {
                throw new functions.https.HttpsError(
                    "failed-precondition",
                    config.soldOutMessage || "?곹뭹??紐⑤몢 ?뚯쭊?섏뿀?듬땲??"
                );
            }
