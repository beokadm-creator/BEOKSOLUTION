...
import { generateBadgePrepToken, type Conference, type Registration } from '../index';
...
describe('badge module', () => {
  ...
  describe('generateBadgePrepToken', () => {
    ...
    it('should generate different tokens on successive calls', () => {
      ...
      for (let i = 0; i < iterations; i++) {
        tokens.add(generateBadgePrepToken());
      }
  ...
  describe('Conference interface', () => {
    ...
    it('should support full conference structure with dates and venue', () => {
      const conference: Conference = {
        ...
        dates: {
          start: undefined as unknown as import('firebase-admin').firestore.Timestamp,
          end: undefined as unknown as import('firebase-admin').firestore.Timestamp,
        },
        venue: {
          name: { ko: '코엑스', en: 'COEX' },
        },
      ...
      expect(typeof conference.venue?.name).toBe('object');
    });
    ...
    it('should support string venue name', () => {
      ...
      expect(typeof conference.venue?.name).toBe('string');
    });
  ...
  describe('Registration interface', () => {
    ...
    it('should support full registration with all fields', () => {
      ...
      expect(registration.userInfo?.name).toBe('김철수');
    });
  ...
  describe('Token expiry calculation logic', () => {
    ...
    it('should set expiry exactly 48 hours after conference end', () => {
      ...
      expect(diffMs).toBe(172_800_000); // exactly 48 hours
    });
  });
  ...
  describe('Payment status filtering', () => {
    it('should allow PAID paymentStatus for regular registrations', () => {
      ...
      const isPaid = isExternal
        ? true
        : regData.paymentStatus === 'PAID' || (regData as { status?: string }).status === 'PAID';

      expect(isPaid).toBe(true);
    });

    it('should allow PAID status field as alternative to paymentStatus', () => {
      const regData = { paymentStatus: undefined as string | undefined, status: 'PAID' };
      const isExternal = false;
      const isPaid = isExternal
        ? true
  ...
  describe('Registration data resolution logic', () => {
    it('should prefer top-level name over userInfo.name', () => {
      const regData = { name: '홍길동', userInfo: { name: '김철수' }, paymentStatus: undefined as string | undefined };
      const name = regData.name || regData.userInfo?.name || '';
      expect(name).toBe('홍길동');
    });

    it('should fall back to userInfo.name when top-level name is missing', () => {
      const regData: { name?: string; userInfo?: { name?: string }; paymentStatus?: string } = { userInfo: { name: '김철수' } };
      const name = regData.name || regData.userInfo?.name || '';
      expect(name).toBe('김철수');
    });

    it('should resolve affiliation from multiple possible fields', () => {
      const regData: { organization?: string; affiliation?: string; userInfo?: { affiliation?: string }; paymentStatus?: string } = {
        organization: '병원',
        affiliation: '대학교',
