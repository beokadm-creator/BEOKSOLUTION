...
export const confirmTossPayment = functions
    ...
    .https.onCall(async (data, _context) => { // eslint-disable-line @typescript-eslint/no-unused-vars
        ...
        try {
            ...
            // 1. Call Toss API
            const approvalResult = await approveTossPayment(paymentKey, orderId, amount, finalSecretKey) as unknown;
            ...
            if (regId && confId) {
                ...
                const paymentMethod = ((approvalResult as { method?: string })?.method) || 'CARD';
