 ...
 */
async function sendAlimTalk(recipient, templateCode, variables, channelId) {
    try {
        ...
        const response = await axios_1.default.post('https://apis.aligo.in/akv10/alimtalk/send', formData, {
            headers: {
 ...
 */
async function getAlimTalkRemain() {
    try {
        const formData = new URLSearchParams();
        formData.append('apikey', ALIGO_CONFIG.apikey);
        formData.append('userid', ALIGO_CONFIG.userid);
        const response = await axios_1.default.post('https://apis.aligo.in/akv10/payment/remain', formData, {
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            }
        });
 ...
 */
async function getAlimTalkHistory(channelId, page = 1, limit = 50) {
    try {
        const formData = new URLSearchParams();
        formData.append('apikey', ALIGO_CONFIG.apikey);
        formData.append('userid', ALIGO_CONFIG.userid);
        ...
        const response = await axios_1.default.post('https://apis.aligo.in/akv10/history/list', formData, {
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            }
        });
 ...
 */
async function getAlimTalkTemplates() {
    try {
        const formData = new URLSearchParams();
        formData.append('apikey', ALIGO_CONFIG.apikey);
        formData.append('userid', ALIGO_CONFIG.userid);
        formData.append('senderkey', ALIGO_CONFIG.senderkey);
        const response = await axios_1.default.post('https://apis.aligo.in/akv10/template/list', formData, {
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            }
        });
