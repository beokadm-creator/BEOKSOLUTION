...
const SocietyHomeSection: React.FC<SocietyHomeSectionProps> = ({ society, language = "ko" }) => {
  const getLocalizedText = (
    text: string | { [lang: string]: string } | undefined | { ko: string; en?: string },
  ) => {
    if (!text) return "";
  ...
  const heroDescription = getLocalizedText(society.description);

  return (
    <section className="relative min-h-[440px] h-auto py-12 bg-slate-900 flex items-center overflow-hidden rounded-3xl">
      <div className="absolute inset-0 opacity-30 bg-[url('https://images.unsplash.com/photo-1511578314322-379afb476865?auto=format&fit=crop&q=80&w=2000')] bg-cover bg-center" />
        ...
        <div className="max-w-3xl space-y-6">
          ...
          <p className="text-xl text-slate-300 font-medium leading-relaxed opacity-80">
            ...
            {heroDescription && (
              <>
                <br />
                <span className="text-sm">{heroDescription}</span>
              </>
            )}
          </p>
