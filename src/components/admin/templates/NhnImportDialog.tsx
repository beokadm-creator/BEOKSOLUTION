...
interface NhnImportDialogProps {
    isOpen: boolean;
...
export default function NhnImportDialog({
    isOpen,
    onOpenChange,
    nhnTemplates,
    onSelectTemplate,
}: NhnImportDialogProps) {
    return (
                    ...
                    <div className="space-y-4">
                        {nhnTemplates.length === 0 ? (
                            ...
                            nhnTemplates.map((tpl: unknown) => (
                                <Card key={tpl.templateCode} className="transition-all hover:bg-slate-50/50 cursor-pointer border-slate-100 group" onClick={() => onSelectTemplate(tpl)}>
