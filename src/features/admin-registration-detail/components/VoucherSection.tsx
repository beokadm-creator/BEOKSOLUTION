...
import type { VoucherTokenStatus } from "../types";

type VoucherLinkSectionProps = {
...
const VoucherLinkSection: React.FC<VoucherLinkSectionProps> = ({
  ...
  const [tokenStatus, setTokenStatus] = useState<VoucherTokenStatus>("loading");
      ...
      {(tokenStatus === "active" || tokenStatus === "issued" || tokenStatus === "expired") && badgePrepUrl && (
        <div className="space-y-3">
          ...
          <div className="p-3 bg-gray-50 rounded-lg border">
            ...
            <p className="text-xs font-mono text-gray-800 break-all bg-white p-2 border mt-1 select-all">
              {badgePrepUrl}
            </p>
          </div>
...
export const VoucherSection: React.FC<Props> = ({ registrationId, effectiveCid, onResend, isResending }) => (
  <VoucherLinkSection
    registrationId={registrationId}
    confId={effectiveCid}
    confBaseUrl={window.location.origin}
    confSlug={effectiveCid && effectiveCid.includes("_") ? effectiveCid.split("_").slice(1).join("_") : effectiveCid || ""}
    onResend={onResend}
