...
import { Registration, LogEntry, ZoneRule } from '../types';
...
export const AttendanceLogModal: React.FC<AttendanceLogModalProps> = ({
    cid,
    registration,
    zones,
    onClose
    ...
    useEffect(() => {
        ...
        const fetchLogs = async () => {
            setLoading(true);
            const collectionName = registration.isExternal ? 'external_attendees' : 'registrations';
            try {
                const q = query(
                    collection(db, 'conferences', cid, collectionName, registration.id, 'logs'),
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[85vh]">
                <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    <div>
                        <h3 className="font-bold text-lg text-slate-900">{registration.userName}</h3>
                        <p className="text-xs text-slate-500 mt-0.5 font-mono">{registration.userEmail}</p>
                    </div>
