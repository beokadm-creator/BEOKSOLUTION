...
const BadgeManagementPage: React.FC = () => {
  ...
  const [demoStep, setDemoStep] = useState<
    "idle" | "scanned" | "consented" | "reward"
  >("idle");
  const [demoGuestbook, setDemoGuestbook] = useState(
    "부스 상담 후 연락을 받고 싶습니다.",
  );
  ...
  useEffect(() => {
    ...
    const fetchSettings = async () => {
      try {
        const [badgeConfigSnap, stampTourSnap, stampSponsorsSnap] =
          await Promise.all([
        ...
        setStampTourConfig(
          stampTourSnap.exists()
            ? (stampTourSnap.data() as StampTourPreviewConfig)
            : null,
        );
  ...
  const demoRewards = useMemo(() => {
    if (stampTourConfig?.rewards?.length) {
      return stampTourConfig.rewards.map(
        (reward) => reward.name || "현장 경품",
      );
    }
      ...
      <div className="mx-auto max-w-6xl px-4 py-8">
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600">
            디지털 명찰, 출력 명찰, 부스 스탬프 투어 DEMO 흐름을 함께
            관리합니다.
          </p>
        </div>
            ...
            <div className="grid gap-6">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    스캔, 동의, 방명록, 스탬프 적립, 추첨 애니메이션 흐름을 미리
                    점검합니다.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid gap-6 lg:grid-cols-[minmax(0,360px)_minmax(0,1fr)]">
                      ...
                      <div className="rounded-[1.5rem] bg-white p-5 text-center">
                        ...
                        <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-left">
                          <div className="flex items-center justify-between text-sm font-bold text-amber-900">
                            <span>스탬프 진행률</span>
                            <span>
                              {demoStamps} / {requiredStampCount || 3}
                            </span>
                          </div>
                          <div className="mt-2 h-2 overflow-hidden rounded-full bg-amber-200">
                            <div
                              ...
                              style={{
                                width: `${Math.min(100, (demoStamps / Math.max(requiredStampCount || 3, 1)) * 100)}%`,
                              }}
                            />
                          </div>
                          <p className="mt-2 text-xs text-amber-700">
                            {stampTourConfig?.completionMessage ||
                              "목표 수량을 달성하면 추첨 버튼이 활성화됩니다."}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid gap-3 md:grid-cols-2">
                        <div
                          className={`rounded-2xl border p-4 ${demoStep === "idle" ? "border-emerald-300 bg-emerald-50" : "border-slate-200 bg-white"}`}
                        >
                          <p className="mb-2 text-sm font-bold text-slate-900">
                            1. 명찰 스캔
                          </p>
                          <p className="mb-3 text-xs text-slate-500">
                            파트너 기기에서 참가자 명찰을 스캔하는 단계입니다.
                          </p>
                          <Button className="w-full" onClick={runDemoScan}>
                            스캔 시뮬레이션
                          </Button>
                        </div>

                        <div
                          className={`rounded-2xl border p-4 ${demoStep === "scanned" || demoStep === "consented" ? "border-emerald-300 bg-emerald-50" : "border-slate-200 bg-white"}`}
                        >
                          <p className="mb-2 text-sm font-bold text-slate-900">
                            2. 동의 및 적립
                          </p>
                          <p className="mb-3 text-xs text-slate-500">
                            개인정보 동의 후 리드 저장과 스탬프 적립이
                            진행됩니다.
                          </p>
                          <Button
                            className="w-full"
                            onClick={runDemoConsent}
                      ...
                      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                        ...
                        <Input
                          value={demoGuestbook}
                          onChange={(event) =>
                            setDemoGuestbook(event.target.value)
                          }
                        />
                        <p className="mt-2 text-xs text-slate-500">
                          입력한 메시지는 파트너 방명록과 참가자 방문 이력에
                          반영됩니다.
                        </p>
                      </div>
                      ...
                      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                        <div className="mb-3 flex items-center justify-between gap-3">
                          <div>
                            ...
                            <p className="text-xs text-slate-500">
                              현재 설정:{" "}
                              {stampTourConfig?.enabled
                                ? `${requiredStampCount}개 달성 시 추첨 가능`
                                : "스탬프 투어 비활성화"}
                            </p>
                          </div>
                        ...
                        <Button
                          className="w-full"
                          onClick={runDemoReward}
                          disabled={demoStamps < (requiredStampCount || 1)}
                        >
                          추첨 애니메이션 실행
                        </Button>
