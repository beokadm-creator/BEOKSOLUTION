...
import { Home, Calendar, Users, CreditCard, MapPin, UserPlus } from 'lucide-react';
...
export const ConferenceWideTemplatePreview = ({ slug }: Props) => {
  ...
  useEffect(() => {
    const fetchTerms = async () => {
      ...
      try {
        ...
        console.error('[ConferenceWideTemplatePreview] Failed to fetch terms:', e);
        setTerms(undefined);
      }
    };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white font-sans text-slate-900">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-eregi-neutral-100 shadow-lg z-50 safe-area-inset-bottom">
        <div className="max-w-7xl mx-auto px-2 sm:px-4">
          <div className="flex items-center justify-between gap-2 py-2 sm:py-3">
            {/* Tab Buttons */}
            <div className="flex justify-around sm:justify-center sm:gap-3 flex-1 overflow-x-auto scrollbar-hide">
              {tabItems.map((item) => {
            ...
            {/* FAB Register Button (Mobile Only) */}
            <button
              type="button"
              onClick={() => setShowRegistrationModal(true)}
              className="flex-shrink-0 w-14 h-14 bg-gradient-to-br from-blue-500 to-blue-600 hover:from-blue-400 hover:to-blue-500 text-white rounded-full shadow-lg shadow-blue-600/30 flex items-center justify-center transition-transform hover:scale-105 active:scale-95 md:hidden"
              aria-label="등록(조회)하기"
            >
...
export default ConferenceWideTemplatePreview;
